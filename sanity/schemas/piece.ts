...
export const pieceType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'number',
      description: 'Base price in the selected currency. Cross-border tax and duties are handled separately.',
    ...
    defineField({
      ...
      options: {
        list: [
          { title: 'Concept only', value: 'concept' },
          { title: 'In talks with maker', value: 'in-talks' },
    ...
    defineField({
      ...
      type: 'string',
      description: 'For example: Ships in 5–7 days, or Made to order in Kyoto.',
    }),
    defineField({
      name: 'waitlistCount',
      title: 'Waitlist Count',
      type: 'number',
      hidden: ({ parent }) => !['concept', 'preorder'].includes(parent?.availability),
